...
class TestGetProductWithDatabase:
    ...
    async def test_get_product_success(self, db_session, test_product, test_offer):
        ...
        # Create the product and offer in database
    ...
    async def test_get_product_without_relationships(self, db_session, test_category, test_brand, test_organization):
    ...
    async def test_get_product_with_multiple_offers(self, db_session, test_product, test_organization):
    ...
    async def test_get_product_with_invalid_offer_data(self, db_session, test_product, test_organization):
...
class TestGetProductsWithDatabase:
    ...
    def test_get_products_success(self, db_session, test_search_results):
        ...
        with patch('app.api.routes.products.SearchService') as mock_service_class:
            ...
            with patch('app.api.routes.products.format_product_search_response') as mock_formatter:
...
class TestProductsAPIEndpointsWithDatabase:
    """Integration tests for products API endpoints using real database"""

    def test_get_product_endpoint_success(self, client, app_with_test_db, test_product, test_offer):
    ...
    def test_get_products_endpoint_success(self, client, app_with_test_db, test_search_results):
    ...
    def test_get_products_endpoint_default_query(self, client, app_with_test_db, test_search_results):
...
class TestProductResponseValidationWithDatabase:
    """Test cases for response validation using real database"""

    async def test_product_detail_response_validation(self, db_session, test_product, test_offer):
    ...
    def test_product_search_response_validation(self, db_session, test_search_results):
...
class TestDatabaseOperations:
    ...
    def test_product_repository_operations(self, db_session, test_product):
        """Test ProductRepository operations with real database"""
        repo = ProductRepository(db_session)
        
        # Test get_by_urn
        product = repo.get_by_urn(test_product.urn)
        assert product is not None
        assert product.id == test_product.id
        assert product.name == test_product.name
        
        # Test get_by_id
        product_by_id = repo.get_by_id(test_product.id)
        assert product_by_id is not None
        assert product_by_id.urn == test_product.urn

    def test_product_relationships_loading(self, db_session, test_product, test_offer):
        """Test that product relationships are properly loaded"""
    ...
    def test_offer_creation_and_retrieval(self, db_session, test_product, test_organization):
        """Test offer creation and retrieval"""
        # Create a new offer
        new_offer = Offer(
            product_id=test_product.id,
            seller_id=test_organization.id,
            ...
            raw_data={
                ...
                "availability": "https://schema.org/InStock",
                "priceCurrency": "USD",
                "inventoryLevel": {"@type": "QuantitativeValue", "value": 15}
            }
        )
        ...
        assert new_offer.id is not None
        assert new_offer.price == 25.99
        assert new_offer.product_id == test_product.id 