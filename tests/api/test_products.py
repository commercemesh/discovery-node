...
class TestProductsAPIEndpoints:
    ...
    def test_get_product_endpoint_success(
        ...
        """Test GET /api/v1/product/{sku_urn} endpoint success with real database."""
        response = client.get(f"/api/v1/product/{test_product.urn}")
    ...
    def test_get_product_endpoint_not_found(self, client, app_with_test_db):
        """Test GET /api/v1/product/{sku_urn} endpoint not found with real database."""
        response = client.get("/api/v1/product/urn:cmp:sku:nonexistent")
    ...
    def test_get_products_endpoint_success(
        ...
        """Test GET /api/v1/products endpoint success with real database."""
    ...
    def test_get_products_endpoint_empty_query(self, client, app_with_test_db):
        """Test GET /api/v1/products endpoint with empty query."""
        response = client.get("/api/v1/products?q=")

        assert response.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY
        data = response.json()
        assert "String should have at least 1 character" in data["errors"][0]["msg"]

    def test_get_products_endpoint_default_query(
        self, client, app_with_test_db, test_search_results
    ):
        """Test GET /api/v1/products endpoint with default query."""
        with patch("app.api.routes.products.SearchService") as mock_service_class:
            mock_service = mock_service_class.return_value
            mock_service.search_products.return_value = test_search_results
            ...
            with patch(
                ...
                response = client.get("/api/v1/products")  # Uses default query "James Cameron"
    ...
    def test_get_products_endpoint_long_query(self, client, app_with_test_db):
        """Test GET /api/v1/products endpoint with query too long."""
        long_query = "a" * 501  # Exceeds max_length of 500
        response = client.get(f"/api/v1/products?q={long_query}")

        assert response.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY

    def test_get_products_endpoint_invalid_query_type(self, client, app_with_test_db, test_search_results):
        """Test GET /api/v1/products endpoint with invalid query type."""
        with patch("app.api.routes.products.SearchService") as mock_service_class:
            mock_service = mock_service_class.return_value
            mock_service.search_products.return_value = test_search_results
            ...
            with patch(
                ...
                response = client.get("/api/v1/products?q=123")  # Query should be string
